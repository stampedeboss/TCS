---------------------------------------------------------------------
-- DCSMIS: CAUCASUS DEFENSES
-- Historical SAM placement and Red Air CAP zones.
---------------------------------------------------------------------
env.info("DCSMIS: Loading Caucasus Defenses")

DCSMIS = DCSMIS or {}
DCSMIS.Caucasus = DCSMIS.Caucasus or {}
...
function DCSMIS.Caucasus.InitDefenses()
  env.info("DCSMIS: Spawning Caucasus Defenses...")

  -- Spawn SAMs
  for _, sam in ipairs(DCSMIS.Caucasus.Defenses.SAMs) do
    local p = DCSMIS.GetRelativePoint(sam.Ref, sam.Bearing, sam.Dist)
    if p and Templates[sam.Type] then
      SpawnGroup(sam.Name, Templates[sam.Type], p, 0)
      env.info("DCSMIS: Spawned " .. sam.Type .. " at " .. sam.Name)
    end
  end

  -- Spawn CAPs
  for _, cap in ipairs(DCSMIS.Caucasus.Defenses.CAP) do
    local p = DCSMIS.GetRelativePoint(cap.Ref, cap.Bearing, cap.Dist)
    if p then
      ...
      env.info("DCSMIS: Spawned CAP " .. cap.Name)
    end
  end
end
