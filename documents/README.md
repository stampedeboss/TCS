...
- persist regardless of player presence
- act as the focal point for all dynamic Air and Ground systems
- **Remain completely map-agnostic**: distances (NM) and geometry (angles) are calculated mathematically, functioning identically on every DCS map without map-specific templates.
...
- **Event-Driven Persistence**: A battle continues as long as events occur, even if the instigating pilot leaves.
- **Unified Geometry**: Air and Ground geometry all converge on the objective realistically.
...
## Air Spawning & Geometry (Authoritative)

### Core Principle
> **Air opponents are spawned dynamically relative to the geographic Zone being contested. Geometry is calculated tactically based on objective locations, not fixed templates.**

Air spawning follows the **same solver pattern as Ground**, replacing terrain constraints with airspace constraints.

---

### Anchor Selection
- The **Objective Zone** or target coordinate is the primary anchor.
- For F10 dynamic spawns, a 50x50nm zone is projected forward along the pilot's ground track, and its centerpoint becomes the anchor.
...
This preserves a single unified Air capability with multiple expressions.

---

### Lifecycle Rules
- Tasks are bound to the Zone's lifecycle, not a player's connection status.
- Despawn when:
  - the zone times out due to a lack of events or player presence for a configurable duration.
  - all assigned units are destroyed or routed.
- Active zones persist indefinitely as long as events are occurring.
- Respawn automatically occurs if the `respawn` parameter is active for the objective.
...
- unified Air/Ground architecture
...
The map becomes the board. Zones become objectives. Capabilities become material.
