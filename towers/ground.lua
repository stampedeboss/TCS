        ...
        for category, count in pairs(params.composition) do
            ...
            if #pool > 0 then
                ...
                while remaining > 0 do
                    ...
                    if TCS.Towers.Ground.Inventory and TCS.Towers.Ground.Inventory.Request(params.coalition, selected.id, groupCount, params.anchor) then
                        ...
                        local item = { types = types, count = groupCount, id = selected.id, formation = "WEDGE" }
            ...
            if #pool > 0 then
                ...
                if type(ratioData) == "table" and ratioData.sub then
                    ...
                    for role, subRatio in pairs(ratioData.sub) do
                        ...
                        if #subPool > 0 then
                            local roleCount = math.max(subRatio > 0 and 1 or 0, math.floor((categoryTotalCount * subRatio) + 0.5))
                            
                            -- Variety Chunking: Split into tactical groups of 4 and re-roll types
                            ...
                            while remaining > 0 do
                                ...
                                -- Handle unit_type (singular) from catalog and standardize for Spawner
                                local types = type(selected.unit_type) == "table" and selected.unit_type or { selected.unit_type }
