...
TCS.Towers.Land.Inventory = { RED = {}, BLUE = {} } -- Initialize inventory pools

--- Deducts units from the motor pool.
-- @param anchor (Coordinate, optional) Used to locate physical warehouse.
function TCS.Towers.Land.Inventory.Consume(side, unitId, count, anchor)
    -- Bypass if logistics is disabled
    if TCS.Common and TCS.Common.Config and TCS.Common.Config.LOGISTICS_ENABLED == false then
        return
    end

    -- 1. Physical Consumption (DCS Warehouse)
    if anchor and TCS.Common and TCS.Common.Warehouse then
    ...
    local current = TCS.Towers.Land.Inventory[pool][unitId] or 999 -- Default high
    
    TCS.Towers.Land.Inventory[pool][unitId] = math.max(0, current - count)
    
    if TCS.Towers.Land.Inventory[pool][unitId] == 0 then
        env.warning(string.format("TCS(LAND.INVENTORY): Motor Pool exhausted for %s", unitId))
    end
end

--- Checks if units are available in the pool.
-- @param anchor (Coordinate, optional) Used to check physical warehouse.
function TCS.Towers.Land.Inventory.Request(side, unitId, count, anchor)
    ...
    local pool = (side == coalition.side.RED) and "RED" or "BLUE"
    local current = TCS.Towers.Land.Inventory[pool][unitId] or 999
...
function TCS.Towers.Land.Inventory.SetSupply(side, unitId, count)
    local pool = (side == coalition.side.RED) and "RED" or "BLUE"
    TCS.Towers.Land.Inventory[pool][unitId] = count
end

env.info("TCS(TOWER.LAND.INVENTORY): ready")