...
function TCS.Catalog.FindById(id, tier)
  ...
  if #types > 0 then return { id = id, unit_types = types } end
  return nil
end
...
function TCS.Catalog.Ingest(data)
    ...
    for category, list in pairs(data.A2G) do
      for _, item in ipairs(list) do
        ...
        if world and world.getDescByName and item.unit_type and not world.getDescByName(item.unit_type) then
          local roleAlts = { LN = "2P25", TR = "1S91 str", SR = "p-19 s-125 sr", AAA = "ZU-23 Emplacement", MBT = "T-55", IFV = "BMP-2", APC = "BTR-80", TRANSPORT = "Ural-375", RIFLEMAN = "Infantry AK", BVR = "MiG-21Bis", AWACS = "A-50", FORTIFICATION = "Container Red 1" }
          local fallbacks = (TCS.CIC and TCS.CIC.Config and TCS.CIC.Config.CatalogFallbacks) or {}
        ...
        local entry = { id = item.id, domain = item.domain, mobile = (item.mobility ~= "STATIC"), late_activation = true, unit_types = { item.unit_type }, first_service_year = item.first_service_year, threat = item.role, tags = { role = item.role, mobility = item.mobility, threat_band = item.threat_band, category = category, coalition = item.coalition }, data = { coalition = item.coalition } }
  ...
  if data.Air_Bandits then
    ...
    for _, b in ipairs(data.Air_Bandits) do
      TCS.Catalog.Add({ id = b.id, domain = "AIR", mobile = true, late_activation = true, unit_types = { b.unit_type }, first_service_year = b.first_service_year, threat = "AIR_FIGHTER", skill_profile = skillMap[b.skill] or "G", tags = { role = b.filters.role, tier = b.filters.tier, type = b.filters.type, coalition = "RED" }, data = { payload = b.payload, livery = b.livery } })
    end
  end
  if data.Air_Packages then
    for _, p in ipairs(data.Air_Packages) do
      TCS.Catalog.Add({ id = p.id, domain = "AIR", mobile = true, late_activation = true, unit_types = { p.unit_type }, threat = "AIR_SUPPORT", first_service_year = p.first_service_year, tags = { role = p.role, coalition = p.coalition or "BLUE" }, data = { count = p.count, skill = p.skill } })
    end
  end
end
