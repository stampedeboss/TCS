...
TCS.Config.Ground = TCS.Config.Ground or {}
TCS.Config.Ground.JTAC = dofile(TCS.GetPath("config/ground/jtac.lua"))
local DefaultMark = TCS.Config.Ground.JTAC.DEFAULT_MARK
local DefaultLaserCode = TCS.Config.Ground.JTAC.DEFAULT_LASER_CODE
...
function TCS.A2G.JTAC.MarkFriendlies(session, groups, mark, laserCode)
  mark = mark or TCS.A2G.JTAC.GetDefaultMark(session)
  laserCode = laserCode or (session.A2G and session.A2G.LaserCode) or DefaultLaserCode
...
function TCS.A2G.JTAC.LaserOn(session, targetCoord, code, duration)
  code = code or (session.A2G and session.A2G.LaserCode) or DefaultLaserCode
  duration = duration or TCS.Config.Ground.JTAC.LASER_DURATION
